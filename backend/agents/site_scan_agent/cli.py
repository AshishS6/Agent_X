#!/usr/bin/env python3
"""
CLI wrapper for Site Scan Agent

This CLI tool is called by the Go backend to execute site scan tasks.
...
site_scan_dir = os.path.join(agents_dir, "site_scan_agent")
sys.path.insert(0, agents_dir)
if site_scan_dir not in sys.path:
    sys.path.insert(0, site_scan_dir)
...
logger = logging.getLogger("SiteScanAgent.CLI")
...
env_paths = [
    os.path.join(os.path.dirname(__file__), '.env'),  # agents/site_scan_agent/.env
...
def run_agent(action: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
    """Execute the site scan agent and return result."""
    ...
    return create_output(
        status="failed",
        error=f"Unsupported action for site scan agent: {action}"
    )


def main():
    parser = argparse.ArgumentParser(
        description="Site Scan Agent CLI",
...
Examples:
  ...
  KYC scan:
    python cli.py --input '{"action": "kyc_site_scan", "url": "https://example.com", "merchant_legal_name": "Example Inc", "registered_address": "123 Main St"}'
